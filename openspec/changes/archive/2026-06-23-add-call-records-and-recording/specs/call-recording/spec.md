# Spec: 整通通话录音归档（agent-flow 自录双声道 + MinIO + artifact 回写）

> 能力：agent-flow 作为音频枢纽自录每通呼入的 caller（upstream）+ AI TTS（downstream）双声道，挂断后间隔 3 秒合成 16kHz 立体声 wav（L=用户 R=AI），上传 MinIO，回写 `call_artifact(kind='recording')`。
>
> **为何不用 FS record_session**：FS `record_session` 录不到 mod_audio_fork 注入的 AI TTS（media bug 叠加在播放层，不在 record 抓的原生帧），实测 R 声道 -91dB 全静音。agent-flow 同时持有 caller 收帧 + AI TTS PCM 两路，自录天然同步、单一来源、无需合并。录音提示音在 dialplan answer 后播放以满足合规，`call_session.recording_notice_played` 标志记录是否播放。

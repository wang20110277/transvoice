# Spec: 通话记录持久化（agent-flow repository 接线）

> 能力：将 agent-flow 通话生命周期（session 开始/结束、每轮 user+assistant 对话、barge-in/handoff 等关键事件）从"零 PG 写入"接入到 `callbot.call_session`/`call_turn`/`call_event` 四表。`call_artifact` 由 call-recording 能力写入。PG 写入与现有 Redis `save_turn` 双写并存（职责不同：Redis 给下一轮 LLM 热上下文，PG 给 console 审查）。所有 PG 写入 fire-and-forget，**绝不阻断通话流**。

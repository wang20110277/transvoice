智能外呼现状：
1-2坐席：营销 获客 1000 100-150  外呼电话 信客 新客有风险 手机号信息客户标签
100坐席：催收
10：客服，消保，官网
目标：
1，产品，话术，营销策略
...
结果：
线索打标系统-人工跟进-剔除失败-增加线索可靠性

FreeSWITCH 原生 `detect_speech` + ESL 回传、RocketMQ、LangChain/LangGraph、Redis+PG17(pgvector)+mem0、NAS+MinIO、三业务强隔离、固定录音告知、GPU隔离、并发池与监控告警等。
---
...
# 现在开始交付“可复制粘贴交付包”第一部分：PostgreSQL 17 DDL（含分区+索引+pgvector HNSW）

你之前我问的3个DDL关键细节你还没回答，这里给出**推荐默认**（如果你不同意，我再调整）：
- user_key：落库保存 `core_user_id:phone_hash` 明文（其中 phone_hash 已加盐不可逆）
- 手机号：不落明文，仅保留脱敏展示字段 `phone_masked`（如 138****1234）
- embedding维度：先用 `1536` 作为默认（如果你们后续换 embedding 维度，需要迁移向量列）

如果接受这些默认，我就按下面DDL执行；如果不接受，请你指出要改哪一项。

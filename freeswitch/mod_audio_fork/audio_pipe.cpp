...
int AudioPipe::lws_callback(struct lws *wsi, 
  ...
  // LWS_CALLBACK_PROTOCOL_INIT creates vhd, so vhd is expected to be NULL here.
  // Must NOT block it — otherwise vhd is never created and all subsequent calls fail.
  if (reason == LWS_CALLBACK_PROTOCOL_INIT) {
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
      "AudioPipe::lws_callback PROTOCOL_INIT context=%p vhd=%p\n", vhd->context, vhd);
    return 0;
  }

  // LWS_CALLBACK_EVENT_WAIT_CANCELLED (reason 72) is called with wsi=NULL
  // in LWS v4.x, so vhd will be NULL. Use static context as fallback.
  if (reason == LWS_CALLBACK_EVENT_WAIT_CANCELLED) {
    if (vhd) {
      ...
      processPendingConnects(nullptr);
      processPendingWrites();
    }
    return 0;
  }
  ...
  if (!vhd) {
    // In client-only contexts (CONTEXT_PORT_NO_LISTEN), LWS may not fire
    // PROTOCOL_INIT for the client connection's vhost, leaving vhd NULL.
    // Create a stack fallback using the static context — safe because
    // lws_callback runs exclusively on the single LWS service thread.
    if (context) {
      static lws_per_vhost_data fallback;
      memset(&fallback, 0, sizeof(fallback));
      fallback.context = context;
      fallback.protocol = lws_get_protocol(wsi);
      fallback.vhost = lws_get_vhost(wsi);
      vhd = &fallback;
      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
        "AudioPipe::lws_callback using fallback vhd for reason %d\n", (int)reason);
    } else {
      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
        "AudioPipe::lws_callback vhd AND static context both NULL for reason %d\n", (int)reason);
      return 0;
    }
  }
  ...
  switch (reason) {
    // LWS_CALLBACK_PROTOCOL_INIT handled above before vhd null check
    ...
    case LWS_CALLBACK_WS_CLIENT_DROP_PROTOCOL:
      ...
      // LWS_CALLBACK_EVENT_WAIT_CANCELLED handled above before vhd null check
      // (LWS v4.x calls this with wsi=NULL, vhd is NULL in that case)
...
void AudioPipe::processPendingConnects(lws_per_vhost_data *vhd) {
  // When vhd is NULL (LWS_CALLBACK_EVENT_WAIT_CANCELLED with wsi=NULL in LWS v4.x),
  // create a temporary vhd using the static context
  lws_per_vhost_data fallback_vhd;
  memset(&fallback_vhd, 0, sizeof(fallback_vhd));
  if (!vhd) {
    if (!context) {
      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "AudioPipe::processPendingConnects both vhd and static context are NULL\n");
      return;
    }
    fallback_vhd.context = context;
    vhd = &fallback_vhd;
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "AudioPipe::processPendingConnects using static context=%p\n", context);
  }
  if (!vhd->context) {
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "AudioPipe::processPendingConnects vhd->context is NULL\n");
    return;
  }
...
bool AudioPipe::connect_client(struct lws_per_vhost_data *vhd) {
  ...
  if (!vhd || !vhd->context) {
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "AudioPipe::connect_client invalid vhd/context, aborting\n");
  ...
  switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
    "AudioPipe::connect_client %s connecting to %s:%u%s (context=%p)\n",
    m_uuid.c_str(), m_host.c_str(), m_port, m_path.c_str(), vhd->context);
  ...
  if (!m_wsi) {
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "AudioPipe::connect_client %s lws_client_connect_via_info returned NULL\n", m_uuid.c_str());
    m_state = LWS_CLIENT_FAILED;
    return false;
  }

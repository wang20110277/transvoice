...
3. **WebSocket音频流层**：FreeSWITCH通过mod_audio_fork建立与agent-flow的WebSocket双向音频流，**上行传输用户语音PCM流，下行接收TTS合成音频**，全程携带CallID与用户手机号。agent-flow运行在**uvloop事件循环**上（libuv C实现），减少高并发下GC停顿
4. **语音识别层**：agent-flow首节点调用agent-asr（内置GPU推理引擎），完成语音转文字，生成**同时携带CallID与用户手机号**的ASR识别文本。支持**gRPC client-streaming**流式传输音频帧（可选，通过`CALLBOT_ASR_USE_GRPC`开关控制），边收边传减少传输延迟
...
```
aiphone/
├── agent-asr/              # ASR 服务 (FastAPI + gRPC, 内置 GPU 推理)
...
├── agent-tts/              # TTS 服务 (FastAPI + gRPC, 内置 GPU 推理)
...
│   ├── dialplan/public.xml    # 拨号计划 + audio_fork WebSocket
...
当前引擎: SenseVoice (ASR, 内置 FunASR GPU 推理), VibeVoice (ASR, 远程 HTTP), CosyVoice (TTS, 内置 CosyVoice3 GPU 推理), VibeVoice (TTS, 远程 HTTP)
...
应用服务组件/
├── agent-asr/
│   ├── asradapter/    # ASR 服务 (FastAPI + gRPC, 内置 GPU 推理, HTTP:8080, gRPC:50051)
│   ├── models/        # SenseVoiceSmall/ 本地模型权重
│   └── Dockerfile     # PyTorch GPU 镜像
├── agent-tts/
│   ├── ttsadapter/    # TTS 服务 (FastAPI + gRPC, 内置 GPU 推理, HTTP:8081, gRPC:50052)
│   ├── models/        # CosyVoice3-0.5B/ 本地模型权重
│   └── Dockerfile     # PyTorch GPU 镜像
├── agent-flow/
│   ├── main.py        # FastAPI 入口
│   ├── src/           # 核心源码 (LangGraph 7 节点 + 流式管道 + WebSocket + ESL + gRPC客户端 + 降噪, port 8000, uvloop)
...
用户来电
    ...
    └─→ FreeSWITCH (mod_sofia)
            │
            ├─→ mod_audio_fork ──→ agent-flow (:8000, uvloop) WebSocket 双向音频
            │       │
            │       ├─→ JitterBuffer → Denoiser → WebRTC VAD → agent-asr (:8080 / gRPC:50051) ──→ SenseVoice GPU0
            │       │                                                       └─→ 返回识别文本
            │       │
            ...
            │       │              └─→ agent-tts (:8081 / gRPC:50052) ──→ CosyVoice GPU1
                    │

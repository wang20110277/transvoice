...
def test_command_starts_with_originate_keyword():
    cmd = build_originate_command(_make_target(), _make_ctx())
...
def test_command_injects_task_and_target_ids():
    cmd = build_originate_command(_make_target(target_id=12), _make_ctx(task_id=5))
    assert "call_task_id=5" in cmd
    assert "call_target_id=12" in cmd


def test_command_injects_user_key():
    cmd = build_originate_command(_make_target(user_key="13800138000"), _make_ctx())
...
def test_command_omits_codec_string_when_empty():
    cmd = build_originate_command(_make_target(), _make_ctx(), codec_string="")
...
def test_command_includes_caller_id_when_set():
    cmd = build_originate_command(_make_target(), _make_ctx(), caller_id="057112345678")
    assert "origination_caller_id_number=057112345678" in cmd


def test_command_omits_caller_id_when_empty():
    cmd = build_originate_command(_make_target(), _make_ctx(), caller_id="")
